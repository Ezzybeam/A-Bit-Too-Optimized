 ...
 * blindness, and darkness fog. Defers to an active shader pack (which sets its own
 * fog).
 */
...
public class FogMixin {
    ...
    // start is kept distinctly below end so the fog band is well-formed.
    ...
    private FogData abto$maybeDisableFog(FogData data) {
        if (RenderToggles.disableFog() && !ShaderCompat.shaderPackActive() && data != null) {
